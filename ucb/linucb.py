...
class LinUCB:
    ...
    def update(self, reward, ratio, arm=None):
        if arm:
            self.a_max = arm
        if reward == -1:
            pass
    ...
    def update_online(self, reward, ratio, arm, user_features):
        xaT = np.array([user_features]).reshape(1, self.d)
        xa = np.transpose(xaT)
        self.x = xa
        self.xT = xaT
    ...
    def recommend(self, timestamp, user_features, articles):
        ...
        # for heatmap
        # self.arm_utility_list.append(np.dot(xaT, theta_tmp) + self.alpha * np.sqrt(np.dot(np.dot(xaT, AaI_tmp), xa)))
    ...
    def save_model(self, path):
        # use pickle
        d = {'Aa': self.Aa, 'ba': self.ba, 'AaI': self.AaI, 'theta': self.theta}
        with open(path, 'wb') as f:
            pickle.dump(d, f)
        # np.savez(path, Aa=self.Aa, ba=self.ba, AaI=self.AaI, theta=self.theta)
